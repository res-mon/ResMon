...
export function groupDeclarationBlocksByClass(
	...
	postCssRoot.each((child) => {
		switch (child.type) {
			...
			case "decl":
				logFunction(
					"Encountered declaration immediately below the postcss root. This shouldn't happen. Ignoring " +
						child.toString(),
				);
				return;
			...
			case "atrule":
				...
				if (child.name === "keyframes") {
					...
					child.walkRules((rule) => {
						try {
							keyframeFrames.push({
								percentage: parseKeyframePercentage(
									rule.selector,
								),
	...
	function handleRule(mediaQuery: string | null, rule: postcss.Rule): void {
		...
		parts.forEach((part, index) => {
			...
			const item =
				recognized.get(elmDeclName) || defaultRecognized(part.class);
	...
	for (const [elmDeclName, declaration] of recognized) {
		const parameterizedDeclaration = isParameterizable(
			elmDeclName,
			declaration,
			colorInfo,
		);
...
function addToSelectorList(
	...
	const index = result.findIndex((elem) =>
		deepEqual(elem.subselectors, subselectors),
	);
...
function everyPartIsSupportedPseudo(
	selector: CssWhat.Selector[],
): selector is SupportedPseudo[] {
	return selector.every(
		(part) => isPseudoSelector(part) || isPseudoElementSelector(part),
	);
}
...
function stripClassSelector(selectorPart: CssWhat.Selector[]): {
	class: string;
...
function parameterizationInfo(
	...
	const possibleColorNames = resolvedColors
		.map(([name, _]) => name)
		.sort((a, b) => b.length - a.length);
	const anyColorName = new RegExp(
		String.raw`(:?.*)(?<colorName>${possibleColorNames.join("|")}).*$`,
	);

	const colorByName: Record<string, ColorInfo> = resolvedColors.reduce(
		(acc, [name, original]) => {
			const parsed = color.parseColor(original);
			const regex =
				parsed != null ? color.colorDetectionRegex(parsed) : null;
...
function isParameterizable(
	...
	declaration: RecognizedDeclaration,
	colorInfo: ParameterizationInfo,
): false | null | [string, ParameterizedDeclaration] {
	...
	const afterColorNamePart = declarationName.substring(
		colorName.length + declarationName.lastIndexOf(colorName),
	);
	...
	const parameterizedDeclaration = {
		...declaration,
		propertiesBySelector: declaration.propertiesBySelector.map(
			(selector) => ({
				...selector,
				properties: selector.properties.map((property) => {
					// Look for unmodified colors appearing as in the config
					const matchIndex = property.value.indexOf(
						resolvedColor.original,
					);
					if (matchIndex >= 0) {
						const matchStartIdx = matchIndex;
						const matchEndIdx =
							matchIndex + resolvedColor.original.length;
						const valuePrefix = property.value.substring(
							0,
							matchStartIdx,
						);
						const valueSuffix =
							property.value.substring(matchEndIdx);
					...
					if (resolvedColor.regex != null) {
						const matchParsed = property.value.match(
							resolvedColor.regex,
						);
						if (matchParsed) {
							const matchStartIdx = matchParsed.index;
							const matchEndIdx =
								matchParsed.index + matchParsed[0].length;
							const valuePrefix = property.value.substring(
								0,
								matchStartIdx,
							);
							...
							const opacity =
								matchParsed?.groups?.varname != null ?
	...
	return [
		`${matches[1]}color`,
		{ ...parameterizedDeclaration, originalColorsReplaced },
	];
}
