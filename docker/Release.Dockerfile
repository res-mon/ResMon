...
RUN chown -R appuser:appuser /app && chmod -R 700 /app
RUN mkdir /app/data && chown -R appuser:appuser /app/data && chmod -R 700 /app/data
RUN touch /app/data/database.db && chown appuser:appuser /app/data/database.db && chmod 700 /app/data/database.db
