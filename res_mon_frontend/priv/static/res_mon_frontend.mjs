...
function assocCollision(root2, shift, hash, key2, val, addedLeaf) {
  if (hash === root2.hash) {
    ...
    return {
      type: COLLISION_NODE,
      hash,
      array: cloneAndSet(root2.array, size2, { type: ENTRY, k: key2, v: val })
    };
  }
  return assoc(
    {
...
function parse_int(value5) {
  if (/^[-+]?(\d+)$/.test(value5)) {
    return new Ok2(parseInt(value5));
  } else {
    return new Error2(Nil2);
  }
}
function parse_float(value5) {
  if (/^[-+]?(\d+)\.(\d+)$/.test(value5)) {
    return new Ok2(parseFloat(value5));
  } else {
    return new Error2(Nil2);
  }
}
...
function string_replace(string3, target3, substitute) {
  if (typeof string3.replaceAll !== "undefined") {
    return string3.replaceAll(target3, substitute);
  }
...
function map_get(map6, key2) {
  const value5 = map6.get(key2, NOT_FOUND);
  if (value5 === NOT_FOUND) {
    return new Error2(Nil2);
  }
  return new Ok2(value5);
}
function map_insert(key2, value5, map6) {
  return map6.set(key2, value5);
}
...
function decode_field(value5, name) {
  const not_a_map_error = () => decoder_error("Dict", value5);
  if (value5 instanceof Dict || value5 instanceof WeakMap || value5 instanceof Map) {
    const entry = map_get(value5, name);
    return new Ok2(entry.isOk() ? new Some(entry[0]) : new None());
  } else if (value5 === null) {
    return not_a_map_error();
  } else if (Object.getPrototypeOf(value5) == Object.prototype) {
    return try_get_field(value5, name, () => new Ok2(new None()));
  } else {
    return try_get_field(value5, name, not_a_map_error);
  }
}
function try_get_field(value5, field4, or_else) {
  try {
    return field4 in value5 ? new Ok2(new Some(value5[field4])) : or_else();
  } catch {
    return or_else();
  }
}
...
function inspectDict(map6) {
  ...
  let first3 = true;
  map6.forEach((value5, key2) => {
    if (!first3)
      body3 = body3 + ", ";
    body3 = body3 + "#(" + inspect(key2) + ", " + inspect(value5) + ")";
    first3 = false;
  });
  return body3 + "])";
}
...
function inspectCustomType(record) {
  const props = Object.keys(record).map((label2) => {
    const value5 = inspect(record[label2]);
...
function insert(dict, key2, value5) {
  return map_insert(key2, value5, dict);
}
...
function divide(a2, b) {
  if (b === 0) {
    ...
    let b$1 = b;
    return new Ok2(divideFloat(a2, b$1));
  }
}
...
function key_set(list, key2, value5) {
  if (list.hasLength(0)) {
    return toList([[key2, value5]]);
  } else if (list.atLeastLength(1) && isEqual(list.head[0], key2)) {
    let k = list.head[0];
    let rest$1 = list.tail;
    return prepend([key2, value5], rest$1);
  } else {
    let first$1 = list.head;
    let rest$1 = list.tail;
    return prepend(first$1, key_set(rest$1, key2, value5));
  }
}
...
var Element = class extends CustomType {
  constructor(key2, namespace2, tag3, attrs, children, self_closing, void$) {
    super();
    this.key = key2;
...
function attribute(name, value5) {
  return new Attribute(name, from(value5), false);
}
function property(name, value5) {
  return new Attribute(name, from(value5), true);
}
  ...
  while (stack4.length) {
    ...
    if (next2.content !== void 0) {
      ...
      iterateElement(next2, (fragmentElement) => {
        stack4.unshift({ prev: prev2, next: fragmentElement, parent });
        prev2 = prev2?.nextSibling;
      });
    } else if (next2.subtree !== void 0) {
      stack4.push({ prev: prev2, next: next2, parent });
    }
  }
  return out;
}
function createElementNode({ prev, next, dispatch: dispatch2, stack: stack4 }) {
  ...
  for (const attr of next.attrs) {
    ...
    if (attr.as_property) {
      ...
      const callback = dispatch2(value5);
      ...
      handlersForEl.set(eventName, callback);
      el2.setAttribute(name, value5);
    } else if (name === "class") {
      className = className === null ? value5 : className + " " + value5;
    } else if (name === "style") {
      style3 = style3 === null ? value5 : style3 + value5;
  ...
  for (const child of next.children) {
    iterateElement(child, (currElement) => {
      if (currElement.key !== void 0 && seenKeys !== null) {
        ...
        stack4.unshift({ prev: prevChild, next: currElement, parent: el2 });
...
function diffKeyedChild(prevChild, child, el2, stack4, incomingKeyedChildren, keyedChildren, seenKeys) {
  ...
  if (keyedChildren.size === 0) {
    iterateElement(child, (currChild) => {
      stack4.unshift({ prev: prevChild, next: currChild, parent: el2 });
      prevChild = prevChild?.nextSibling;
    });
    return prevChild;
  }
...
var LustreClientApplication2 = class _LustreClientApplication {
  ...
  #update = null;
  #view = null;
  static start(flags, selector, init6, update5, view3) {
    if (!is_browser())
      return new Error2(new NotABrowser());
    const root2 = selector instanceof HTMLElement ? selector : document.querySelector(selector);
    if (!root2)
      return new Error2(new ElementNotFound(selector));
    const app = new _LustreClientApplication(init6(flags), update5, view3, root2);
    return new Ok2((msg) => app.send(msg));
  }
  constructor([model, effects], update5, view3, root2 = document.body, isComponent = false) {
    this.#model = model;
    this.#update = update5;
    this.#view = view3;
    this.#root = root2;
...
var App = class extends CustomType {
  constructor(init6, update5, view3, on_attribute_change) {
    super();
    this.init = init6;
    this.update = update5;
    this.view = view3;
...
function application(init6, update5, view3) {
  return new App(init6, update5, view3, new None());
}
      ...
      return then$(
        ...
        (g) => {
          return then$(
            ...
            (b) => {
              return new Ok2(new Rgba(r, g, b, 1));
            }
          );
        }
      );
...
function to_css_rgba_string(colour) {
  ...
  return join2(
    toList([
      "rgba(",
      to_string2(percent2(r)) + "%,",
      to_string2(percent2(g)) + "%,",
      to_string2(percent2(b)) + "%,",
      to_string2(round_to(a2)),
      ")"
    ]),
    ""
  );
...
var Theme = class extends CustomType {
  constructor(space, text4, radius, primary, greyscale, error5, warning, success2, info) {
    super();
    this.space = space;
    this.text = text4;
    this.radius = radius;
...
function value_to_css(value5) {
  if (value5 instanceof Px) {
    let val = value5[0];
    return to_string2(val) + "px";
  } else if (value5 instanceof Rem) {
    let val = value5[0];
    return to_string2(val) + "rem";
  } else {
    let name = value5[0];
    return "var(" + name + ")";
  }
...
function size_to_dict(size2, prefix) {
  return from_list(
    toList([
      ...
      ["$(" + prefix + ".ratio)", to_string2(size2.ratio)]
    ])
  );
}
function scale_to_dict(scale, prefix) {
  ...
  return from_list(map22(keys2, vals, new$2));
}
...
function scoped(theme, selector) {
  ...
  let _pipe$2 = replace(_pipe$1, ":root", selector);
  return ((_capture) => {
...
function getFunctionName() {
  ...
  const stack4 = error5.stack ?? "";
  const parts = stack4.split("\n");
...
function deepEqual(a2, b) {
  ...
  if (consts.includes(typeof a2) || consts.includes(typeof b))
    return a2 === b;
  for (const value5 in a2) {
    if (!(value5 in b))
      return false;
    const isSame = deepEqual(a2[value5], b[value5]);
    if (!isSame)
      return false;
  }
  return true;
...
function setAttribute(element2, name, value5) {
  element2.setAttribute(name, value5);
}
...
function shadow2(root2) {
  return new Options3(new Shadow(root2));
}
...
var Property = class extends CustomType {
  constructor(key2, value5, important) {
    super();
    this.key = key2;
    this.value = value5;
...
function compute_property(indent2, key2, value5, important) {
  ...
  return base_indent + key2 + ": " + value5 + important_ + ";";
}
...
function handle_property(props, style3) {
  ...
  let key2 = style3.key;
  let value5 = style3.value;
  let important = style3.important;
  let css_property = compute_property(props.indent, key2, value5, important);
...
var Px2 = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
var Rem2 = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
function px(value5) {
  return new Px2(to_float(value5));
}
function percent(value5) {
  return new Pct(to_float(value5));
}
function to_string8(size2) {
  if (size2 instanceof Px2) {
    let value5 = size2[0];
    return append4(to_string2(value5), "px");
  } else if (size2 instanceof Pt) {
    let value5 = size2[0];
    return append4(to_string2(value5), "pt");
  } else if (size2 instanceof Pct) {
    let value5 = size2[0];
    return append4(to_string2(value5), "%");
  } else if (size2 instanceof Vh) {
    let value5 = size2[0];
    return append4(to_string2(value5), "vh");
  } else if (size2 instanceof Vw) {
    let value5 = size2[0];
    return append4(to_string2(value5), "vw");
  } else if (size2 instanceof Em) {
    let value5 = size2[0];
    return append4(to_string2(value5), "em");
  } else if (size2 instanceof Rem2) {
    let value5 = size2[0];
    return append4(to_string2(value5), "rem");
  } else if (size2 instanceof Lh) {
    let value5 = size2[0];
    return append4(to_string2(value5), "lh");
  } else if (size2 instanceof Rlh) {
    let value5 = size2[0];
    return append4(to_string2(value5), "rlh");
  } else {
    let value5 = size2[0];
    return append4(to_string2(value5), "ch");
  }
}
...
function property2(field4, content) {
  return new Style(new Property(field4, content, false));
}
function width2(width3) {
  return property2("width", to_string8(width3));
}
function width_(width3) {
  return property2("width", width3);
}
function max_width_(width3) {
  return property2("max-width", width3);
}
function max_height_(height2) {
  return property2("max-height", height2);
}
function color(color2) {
  return property2("color", color2);
}
function font_family2(font_family3) {
  return property2("font-family", font_family3);
}
function font_size2(font_size3) {
  return property2("font-size", to_string8(font_size3));
}
function line_height2(line_height3) {
  return property2("line-height", line_height3);
}
...
function border_bottom2(border_bottom3) {
  return property2("border-bottom", border_bottom3);
}
function border_right2(border_right3) {
  return property2("border-right", border_right3);
}
...
function margin2(margin3) {
  return property2("margin", to_string8(margin3));
}
...
function to_lustre(class$3) {
  ...
  let _pipe$3 = map2(_pipe$2, (value5) => {
    return [value5, true];
  });
...
var Colors = class extends CustomType {
  constructor(background2, shadow3, primary, editor_fg, editor_bg, gutter, syntax_comment, button5, function$, nil, bool3, constant, bit_array, utf_codepoint, string3, number, custom_type, regex, date) {
    super();
...
function inspectDict2(map6) {
  const data = [];
  map6.forEach((value5, key2) => data.push([inspect3(key2), inspect3(value5)]));
...
function inspectCustomType2(record) {
  const props = List.fromArray(
    Object.keys(record).map((label2) => {
      const value5 = inspect3(record[label2]);
      return isNaN(parseInt(label2)) ? [new Some(label2 + ": "), value5] : [new None(), value5];
    })
  );
...
function bordered_grid_header() {
  let _pipe = class$2(
    toList([
      ...
      border_bottom2("2px solid var(--primary)")
    ])
  );
  return to_lustre(_pipe);
}
...
function details_() {
  return class$2(
    toList([
      ...
      font_size2(px(14)),
      border_bottom2("1px solid var(--gutter)")
    ])
  );
}
function details() {
...
function step_index() {
  let _pipe = class$2(
    toList([
      padding_("9px 9px"),
      justify_self("end"),
      border_right2("1px solid var(--gutter)"),
      font_family2("Lexend"),
...
function select_cs() {
  let _pipe = class$2(
    toList([
      ...
      width2(percent(100)),
      font_size2(px(12)),
      font_family2("inherit"),
      line_height2("inherit"),
      color("var(--primary)"),
...
function view_data_line(indent2, prefix, text4, color2) {
  let idt = repeat2(" ", indent2);
  let text_color2 = text_color(color2);
  let $ = length2(prefix);
  if ($ === 0) {
    return div(toList([text_color2]), toList([text2(idt + text4)]));
  } else {
    return div(
      toList([flex()]),
      toList([
        div(toList([keyword_color()]), toList([text2(idt + prefix)])),
        div(toList([text_color2]), toList([text2(text4)]))
      ])
    );
  }
}
...
function start_sketch(root2) {
  let setup$1 = setup(shadow2(root2));
  return map_error(
    setup$1,
    (error5) => {
      debug("Unable to start sketch. Check your configuration.");
      debug(error5);
      return new SketchError(error5);
    }
  );
}
function init4(_) {
...
function setup2() {
  ...
  let _pipe$2 = map3(
    _pipe$1,
    (_capture) => {
      return application(init4, update3, _capture);
    }
  );
  let _pipe$3 = try$(
    _pipe$2,
...
// build/dev/javascript/res_mon_frontend/res_mon_frontend.mjs
...
function update4(theme, msg) {
  return [
    (() => {
      if (msg instanceof UserChangedSpaceBase) {
        let value5 = msg[0];
        return theme.withFields({
          space: theme.space.withFields({ base: new Rem(value5) })
        });
      } else if (msg instanceof UserChangedSpaceRatio) {
        let value5 = msg[0];
        return theme.withFields({
          space: theme.space.withFields({ ratio: value5 })
        });
      } else if (msg instanceof UserChangedTextBase) {
        let value5 = msg[0];
        return theme.withFields({
          text: theme.text.withFields({ base: new Rem(value5) })
        });
      } else if (msg instanceof UserChangedTextRatio) {
        let value5 = msg[0];
        return theme.withFields({ text: theme.text.withFields({ ratio: value5 }) });
      } else {
        let value5 = msg[0];
        return theme.withFields({ radius: new Px(value5) });
      }
    })(),
    none()
  ];
              ...
              toList([
                ...
                on2(
                  ...
                  (event2) => {
                    ...
                    let value5 = $[0];
                    let $1 = (() => {
                      let _pipe = parse(value5);
                      return lazy_or(
                        _pipe,
                        () => {
                          let _pipe$1 = parse2(value5);
                    ...
                    let value$1 = $1[0];
                    return new Ok2(new UserChangedSpaceBase(value$1));
                  }
                )
              ])
            ),
          ...
          aside2(
            ...
            input(
              toList([
                ...
                attribute("max", "3.0"),
                attribute("step", "0.01"),
                attribute("value", to_string2(theme.space.ratio)),
                on2(
                  "input",
                  (event2) => {
                    let $ = value2(event2);
                    ...
                    let value$1 = $1[0];
                    return new Ok2(new UserChangedSpaceRatio(value$1));
                  }
                )
              ])
            ),
              ...
              toList([
                ...
                on2(
                  ...
                  (event2) => {
                    ...
                    let value$1 = $1[0];
                    return new Ok2(new UserChangedTextBase(value$1));
                  }
                )
              ])
            ),
          ...
          aside2(
            ...
            input(
              toList([
                ...
                attribute("max", "3.0"),
                attribute("step", "0.01"),
                attribute("value", to_string2(theme.text.ratio)),
                on2(
                  "input",
                  (event2) => {
                    let $ = value2(event2);
                    ...
                    let value$1 = $1[0];
                    return new Ok2(new UserChangedTextRatio(value$1));
                  }
                )
              ])
            ),
              ...
              toList([
                ...
                on2(
                  ...
                  (event2) => {
                    ...
                    let value$1 = $1[0];
                    return new Ok2(new UserChangedRadius(value$1));
                  }
                )
              ])
            ),
...
function main() {
  ...
  let main$1 = $[0];
  let $1 = (() => {
    let _pipe = application(init5, update4, view2);
