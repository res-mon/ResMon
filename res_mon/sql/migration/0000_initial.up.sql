...
WITH
  "activity_log_with_lead" ("timestamp_seconds", "active", "automatic", "lead_active", "lead_automatic") AS (
    SELECT
      "timestamp" / 1000000.0 AS "timestamp_seconds",
      "active"                                      ,
      "automatic"                                   ,
      LEAD("active") OVER     (
        ORDER BY
  ...
  "extended_activity_log" ("timestamp_seconds", "active", "is_unknown") AS (
    ...
    WHERE
      ...
      (
        ...
        "active" = 1 AND
        STRFTIME('%Y-%m-%d', DATETIME(DATETIME(), 'localtime')) != STRFTIME('%Y-%m-%d', DATETIME(DATETIME("timestamp_seconds", 'unixepoch'), 'localtime'))
      )
    UNION ALL
    SELECT
